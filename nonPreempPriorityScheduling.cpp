...
int findProcToExec( int arr[], int leftBurst[], int burst[], int priority[] ,int currTime, int pid ,int n)
{
...
void schedule(int arr[], int burst[], int finish[], bool isComplete[], int prioirty[], int n)
{
    ...
    for (int i = 0; i < totTime; i++)
    {
        ...
        {
            pid = findProcToExec(arr, leftBurst, burst,prioirty, i, pid , n);
            if (pid != -1)
            {
                leftBurst[pid]--;
                if (leftBurst[pid] == 0)
