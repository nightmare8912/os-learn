...
int findProcToExec(int currTime, int arr[], int leftBurst[], int burst[], int pid, int n)
{
    if (pid != -1 && leftBurst[pid] > 0)
        return pid;
    else
    {
        int min = 999999;
        for (int i = 1; i <= n; i++)
        {
            if (leftBurst[i] > 0 && currTime >= arr[i] && leftBurst[i] <= min)
            {
                min = burst[i];
                pid = i;
            }
...
{
    sort(arr, burst, n);
    int leftBurst[n], pid = -1;
    for (int i = 1; i <= n; i++)
        leftBurst[i] = burst[i];
    int totTime = findSum(burst, n);
    for (int i = 0; i < totTime; i++)
    {
        if (isIdle(leftBurst, arr, i, n) == false)
        {
            pid = findProcToExec(i, arr, leftBurst, burst, pid, n);
            if (pid != -1)
            {
                leftBurst[pid]--;
                if (leftBurst[pid] == 0)
